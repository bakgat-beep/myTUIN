...
class GrowingSpaceHistoryRepositoryTest {
    ...
    @Test
    fun no_history_exists_until_first_edit() =
        runBlocking {
            assertEquals(0, historyRepository.getHistoryForSpace(spaceId).size)
        }

    @Test
    fun edit_writes_one_history_row_capturing_prior_state() =
        runBlocking {
            val newGeometry =
                Geometry.Polygon(
                    listOf(
    ...
    @Test
    fun second_edit_chains_valid_from_to_previous_valid_to() =
        runBlocking {
    ...
    @Test
    fun history_is_not_written_when_editing_a_nonexistent_space() =
        runBlocking {
            try {
                spaceRepository.updateGeometry(
    ...
    private fun sampleSpaceWithInitialGeometry(): GrowingSpace =
        GrowingSpace(
            id = spaceId,
            gardenId = gardenId,
            name = "Bed 2",
