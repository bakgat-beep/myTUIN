...
class CreateGardenTest {
    ...
    @Test
    fun invoke_persists_a_garden_with_assigned_id_status_and_timestamps() = runBlocking {
    ...
    @Test
    fun invoke_defaults_hemisphere_to_unknown_when_not_supplied() = runBlocking {
        createGarden(NewGarden(name = "Back Garden"))
        assertEquals(Hemisphere.UNKNOWN, repository.snapshot().first().hemisphere)
    }

    @Test
    fun invoke_preserves_supplied_hemisphere() = runBlocking {
    ...
    @Test
    fun invoke_preserves_optional_location_fields() = runBlocking {
        createGarden(
            NewGarden(
                name = "Back Garden",
    ...
    @Test
    fun invoke_allows_two_gardens_with_the_same_name() = runBlocking {
        createGarden(NewGarden(name = "Back Garden"))
        createGarden(NewGarden(name = "Back Garden"))
